...
const useRecruitCommentPagination = <T>() => {
    // state: 페이징 관련 상태 //
    ...
    const onPageClickHandler = (page: number) => {
        setCurrentPage(page);
        initViewList(totalList); // 페이지 클릭 시 리스트를 업데이트
    };
    ...
    const onPreSectionClickHandler = () => {
        ...
        const newPage = (currentSection - 1) * PAGES_PER_SECTION; // 새로운 페이지 계산
        setCurrentPage(newPage);
        initViewList(totalList); // 페이지 변경 시 리스트를 업데이트
    };
...
export default useRecruitCommentPagination;